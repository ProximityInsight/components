...
public class MarketoInputReader extends AbstractBoundedReader<IndexedRecord> {
    ...
    public MarketoRecordResult executeOperation(String position) throws IOException {
        ...
        case Campaign:
            switch (properties.campaignAction.getValue()) {
            case get:
                // return ((MarketoRESTClient) client).getCampaigns(properties, position);
            case getByID:
                // return ((MarketoRESTClient) client).getCampaignById(properties);
            }
            break;
        }
